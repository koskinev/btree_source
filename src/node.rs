...
impl<'a, K: 'a, V: 'a, Type> NodeRef<marker::Immut<'a>, K, V, Type> {
    ...
    pub fn keys(&self) -> &[K] {
        ...
        unsafe {
            ...
            // See: https://doc.rust-lang.org/src/core/mem/maybe_uninit.rs
...
impl<'a, K, V, Type> NodeRef<marker::ValMut<'a>, K, V, Type> {
    ...
    unsafe fn into_key_val_mut_at(mut self, idx: usize) -> (&'a K, &'a mut V) {
        ...
        let keys = ptr::addr_of!((*leaf).keys);
        let vals = ptr::addr_of_mut!((*leaf).vals);
        // We must coerce to unsized array pointers because of Rust issue #74679.
        let keys: *const [_] = keys;
        let vals: *mut [_] = vals;
        // let key = unsafe { (&*keys.get_unchecked(idx)).assume_init_ref() };
        // let val = unsafe { (&mut *vals.get_unchecked_mut(idx)).assume_init_mut() };
        let key_ptr = (keys as *const MaybeUninit<K>).add(idx);
        let val_ptr = (vals as *mut MaybeUninit<V>).add(idx);
        let key = (&*key_ptr).assume_init_ref();
        let val = (&mut *val_ptr).assume_init_mut();
        (key, val)
    }
}
...
fn splitpoint(edge_idx: usize) -> (usize, LeftOrRight<usize>) {
    ...
    match edge_idx {
        ...
        i if i < EDGE_IDX_LEFT_OF_CENTER => (KV_IDX_CENTER - 1, LeftOrRight::Left(edge_idx)),
        ...
        unsafe {
            ...
            *parent_node.len_mut() -= 1;
            ...
            if parent_node.height > 1 {
                ...
                let ptr = right_node.node.cast();
                dealloc(ptr.as_ptr(), Layout::new::<InternalNode<K, V>>())
            } else {
                // Global.deallocate(right_node.node.cast(), Layout::new::<LeafNode<K, V>>());
                let ptr = right_node.node.cast();
                dealloc(ptr.as_ptr(), Layout::new::<LeafNode<K, V>>())
            }
        }
...
impl<'a, K, V, Type> Handle<NodeRef<marker::Mut<'a>, K, V, marker::LeafOrInternal>, Type> {
    ...
    pub unsafe fn cast_to_leaf_unchecked(
        ...
        let node = self.node.cast_to_leaf_unchecked();
        Handle {
            node,
            idx: self.idx,
            _marker: PhantomData,
