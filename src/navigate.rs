...
impl<'a, K, V> LazyLeafRange<marker::Immut<'a>, K, V> {
    ...
    pub unsafe fn next_unchecked(&mut self) -> (&'a K, &'a V) {
        self.init_front().unwrap().next_unchecked()
    }

    #[inline]
    pub unsafe fn next_back_unchecked(&mut self) -> (&'a K, &'a V) {
        self.init_back().unwrap().next_back_unchecked()
    }
}
...
impl<K, V> LazyLeafRange<marker::Dying, K, V> {
    ...
    pub unsafe fn deallocating_next_unchecked(
        ...
        front.deallocating_next_unchecked()
    }
    ...
    pub unsafe fn deallocating_next_back_unchecked(
        ...
        back.deallocating_next_back_unchecked()
    }
        ...
        R: RangeBounds<Q>,
    {
        match self.search_tree_for_bifurcation(&range) {
            ...
            Ok((
                ...
                let mut lower_edge = Handle::new_edge(ptr::read(&node), lower_edge_idx);
                let mut upper_edge = Handle::new_edge(node, upper_edge_idx);
                loop {
    ...
    unsafe fn deallocating_next(
        ...
        loop {
            edge = match edge.right_kv() {
                ...
                Err(last_edge) => match last_edge.into_node().deallocate_and_ascend() {
